...
interface RawStory extends Partial<PFCAgileItem> {
    epic?: { name: string }
}

interface RawTask extends Partial<PFCAgileItem> {
    story?: { name: string }
}
...
export const PrefrontalCortex = ({onItemSelect, selectedItemId}: PrefrontalCortexProps) => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (storyRes.ok) {
                ...
                const stories = (data.results || data).map((s: RawStory) => ({
                    ...s,
                    item_type: 'STORY',
                    parent_name: s.epic?.name
                })) as PFCAgileItem[];
                allItems = [...allItems, ...stories];
            }
            if (taskRes.ok) {
                const data = await taskRes.json();
                const tasks = (data.results || data).map((t: RawTask) => ({
                    ...t,
                    item_type: 'TASK',
                    parent_name: t.story?.name
                })) as PFCAgileItem[];
    ...
    const getPriorityIcon = (priority?: number) => {
        switch (priority) {
            case 1:
                return <span title="P1: Critical"><ChevronsUp size={14} color="#ef4444"/></span>;
            case 2:
                return <span title="P2: High"><ChevronUp size={14} color="#f99f1b"/></span>;
            case 3:
                return <span title="P3: Normal"><Minus size={14} color="#94a3b8"/></span>;
            case 4:
                return <span title="P4: Low"><ChevronDown size={14} color="#64748b"/></span>;
            default:
                return null;
        }
    };
    ...
    return (
        <div className="pfc-container">
            ...
            <div className="pfc-board-wrapper">
                {canScrollLeft ?
                    <button className="pfc-scroll-btn" onClick={() => scrollBoard('left')}><ChevronLeft size={24}/>
                    </button> : <div style={{width: '36px', flexShrink: 0}}/>}
                            ...
                            >
                                <div className="pfc-column-header">
                                    <span className="pfc-column-title"
                                          style={{color: isDragOver ? 'var(--accent-purple)' : 'inherit'}}>{status.name}</span>
                                    ...
                                    {columnItems.map(item => {
                                        ...
                                        return (
                                            ...
                                            >
                                                    ...
                                                    <div>
                                                        <div style={{
                                                            ...
                                                            {item.parent_name && <span
                                                                className="font-mono text-xs text-muted">of {item.parent_name.substring(0, 15)}...</span>}
                                                        </div>
                                                ...
                                                {item.description && (
                                                    <div className="font-mono text-xs text-muted" style={{
                                                        display: '-webkit-box',
                                                        WebkitLineClamp: 2,
                                                ...
                                                <div className="pfc-card-meta">
                                                    ...
                                                    {item.owning_disc ? (
                                                        <span className="pfc-tag font-mono" style={{
                                                            ...
                                                            alignItems: 'center',
                                                            gap: '4px'
                                                        }}><Cpu size={10}/> {item.owning_disc.name}</span>
                                                    ) : (
                                                        <span className="pfc-tag font-mono" style={{
                                                            ...
                                                            alignItems: 'center',
                                                            gap: '4px'
                                                        }}><Cpu size={10}/> Unassigned</span>
                                                    )}

                                                    {item.previous_owners && item.previous_owners.length > 0 && (
                                                        <span className="pfc-tag font-mono"
                                                              title={`Previously touched by: ${item.previous_owners.map(p => p.name).join(', ')}`}
                                                              style={{
                                                                  color: 'var(--text-secondary)',
                                                                  display: 'flex',
                                                                  alignItems: 'center',
                                                                  gap: '4px'
                                                              }}><History size={10}/> {item.previous_owners.length}</span>
                                                    )}

                                                    {item.complexity !== undefined && item.complexity > 0 && (
                                                        <span className="pfc-tag font-mono"
                                                              style={{color: 'var(--accent-green)'}}>CX: {item.complexity}</span>
                                                    )}
                                                </div>
                                            </div>
                                        );
                ...
                {canScrollRight ?
                    <button className="pfc-scroll-btn" onClick={() => scrollBoard('right')}><ChevronRight size={24}/>
                    </button> : <div style={{width: '36px', flexShrink: 0}}/>}
            </div>
        </div>
    );
